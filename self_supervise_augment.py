...
import torch
# image size: 128 * 64 *3
path = 'data/market/pytorch/train_all_self'
...
for i in range(int(len(dirs) / 2)):
    ...
    if not os.path.exists(dir_path):
        ...
        for j in range(num):
            ...
            file_name = dirs[i] + dirs[len(dirs) - 1 - i] + str(j) + 'c' + files1[j].split('c')[1]
            cv2.imwrite(os.path.join(dir_path, file_name), img_new)
